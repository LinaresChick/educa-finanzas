...
DROP TABLE IF EXISTS `estudiante_padre`;
...
DROP TABLE IF EXISTS `facturas_recibos`;
...
CREATE TABLE `usuarios` (
  ...
  `rol` enum('Superadmin','Administrador','Secretario','Contador','Colaborador','Padre','Estudiante','Docente') NOT NULL,
  `estado` enum('activo','inactivo') DEFAULT 'activo',
  `fecha_creacion` timestamp NOT NULL DEFAULT current_timestamp(),
  `fecha_actualizacion` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp()
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
-- Placeholder para tabla `costos` (se agregó porque falta su CREATE TABLE en este volcado)
...
-- Placeholder para tabla `deudas` (se agregó porque falta su CREATE TABLE en este volcado)
...
--
ALTER TABLE `estudiantes`
  ADD CONSTRAINT `fk_estudiante_salon` FOREIGN KEY (`id_salon`) REFERENCES `salones` (`id_salon`) ON DELETE SET NULL,
...
--
-- Filtros para la tabla `facturas_recibos`
--
ALTER TABLE `facturas_recibos`
...
--
ALTER TABLE `salones`
  ADD CONSTRAINT `salon_ibfk_1` FOREIGN KEY (`id_grado`) REFERENCES `grados` (`id_grado`),
  ADD CONSTRAINT `salon_ibfk_2` FOREIGN KEY (`id_seccion`) REFERENCES `secciones` (`id_seccion`),
  ADD CONSTRAINT `salon_ibfk_3` FOREIGN KEY (`id_docente`) REFERENCES `docentes` (`id_docente`) ON DELETE SET NULL;
COMMIT;
